...
export default function ResultsTable({
  ...
  const trArray = submittedNames
    ...
    .reverse()
    .map((name) => {
      const ammoMatch = checkMatching(
        GUNDATA[name].ammoType,
        secretGunData.ammoType
      );
      const attachmentMatch = checkMatching(
        GUNDATA[name].attachments,
        secretGunData.attachments
      );
      const firingModeMatch = checkMatching(
        GUNDATA[name].firingMode,
        secretGunData.firingMode
      );
      ...
      return (
        <tr key={name}>
            ...
            style={{
              backgroundColor:
                ammoMatch === "all"
                  ...
                  : ammoMatch === "none"
                  ? "rgba(255, 0, 0, 0.59)"
                  : "rgba(219, 132, 13, 0.59)",
            }}
          >
            {formatList(GUNDATA[name].ammoType)}
          </td>
          <td
            style={{
              backgroundColor:
          ...
          >
            Season {GUNDATA[name].releaseDate}{" "}
            {secretGunData.releaseDate > GUNDATA[name].releaseDate
              ? "▲"
              : secretGunData.releaseDate < GUNDATA[name].releaseDate
              ? "▼"
              : ""}
          ...
          >
            {GUNDATA[name].damagePerSecond}{" "}
            {secretGunData.damagePerSecond > GUNDATA[name].damagePerSecond
              ? "▲"
              : secretGunData.damagePerSecond < GUNDATA[name].damagePerSecond
              ? "▼"
              : ""}
          ...
          <td
            style={{
              backgroundColor:
                firingModeMatch === "all"
                  ? "rgba(0, 255, 0, 0.59)"
                  : firingModeMatch === "none"
                  ? "rgba(255, 0, 0, 0.59)"
                  : "rgba(219, 132, 13, 0.59)",
            }}
          >
            {formatList(GUNDATA[name].firingMode)}
          </td>
        </tr>
      );
    });
